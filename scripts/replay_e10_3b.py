"""New-process exact replay for registered HLM5 E10."""
...
from hlm5.e10_runtime import (
    MODEL_FORWARD_BATCH, capacity_measurement, e10_environment_record, load_bundle,
)
...
def run_replay(
    *, started: float, admission: dict[str, Any], admission_sha: str,
    ...
    if frozen_bundle is not None:
        if portable is None:
            raise RuntimeError("E10 replay did not reconstruct the portable bundle")
...
def main() -> None:
    ...
    try:
        if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
            raise RuntimeError("registered E10 replay requires exactly one GPU")
        torch.manual_seed(SEED)
        device = torch.device("cuda", 0)
        native_runtime = configure_native_runtime()
        if native_runtime != receipt["native_runtime"]:
            raise RuntimeError("E10 replay arithmetic differs from registration")
        if e10_environment_record(device) != receipt["environment"]:
            raise RuntimeError("E10 replay environment differs from registration")
        run_replay(
