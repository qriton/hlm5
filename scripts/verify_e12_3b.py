"""Emit the formal fail-closed HLM5 E12 verdict."""
...
from hlm5.e12_contract import (
    ADMISSION_PATH, ATTEMPT_PATH, RESULT_PATH, VERDICT_PATH, VERDICT_SCHEMA,
    atomic_create_json, load_admission, load_attempt, load_json_object, load_result,
    scientific_failures, scientific_sha256, verdict_scientific_payload,
)
...
def main() -> int:
    if VERDICT_PATH.exists():
        raise FileExistsError(f"E12 verdict already exists: {VERDICT_PATH}")
    validity: list[str] = []
    ...
    if not RESULT_PATH.is_file():
        ...
        if validity:
            ...
            verdict, exit_code = "FAIL_3B_UNTOUCHED_FINAL", 0
        else:
            verdict, exit_code = "PASS_3B_UNTOUCHED_FINAL", 0
    scientific = verdict_scientific_payload(
        verdict=verdict, result=result, validity_failures=validity,
    ...
    payload = {
        ...
        "admission_scientific_sha256": None if admission is None else admission.get("scientific_sha256"),
        "result_sha256": result_sha or optional_hash(RESULT_PATH),
        "result_scientific_sha256": None if result is None else result.get("scientific_sha256"),
