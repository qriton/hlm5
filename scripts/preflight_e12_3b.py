"""Outcome-blind preflight for registered HLM5 E12."""
...
from hlm5.e12_runtime import (
    FINAL_CASE_START, CAPACITY_TIERS, GATE_BATCH, MEMORY_SIZE,
    MODEL_FORWARD_BATCH, e12_environment_record,
)
...
def main() -> None:
    ...
    if tracked != set(source_sha256):
        raise RuntimeError("every registered E12 source must be tracked")
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("registered E12 preflight requires exactly one GPU")
    ...
    from hlm5.e12_contract import load_json_object, REPO_ROOT, E8_RESULT_PATH
    ...
    if failures:
        raise RuntimeError("registered E12 environment mismatch: " + "; ".join(failures))
    payload = {
        ...
        "candidate_start": FINAL_CASE_START,
