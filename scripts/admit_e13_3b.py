"""Run the single-use E13 nested-capacity admission."""
...
def run_after_attempt(
    *,
    ...
    measurement, bundle = terminal_measurement(
        prepared["model"],
        population["cases"],
        exact_hidden,
        whitening_hidden,
...
def main() -> None:
    started = time.time()
    assert_outputs_absent(
        [ATTEMPT_PATH, BUNDLE_PATH, ADMISSION_PATH, RESULT_PATH, VERDICT_PATH]
    )
    receipt, receipt_sha = load_execution_receipt()
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("registered E13 admission requires exactly one GPU")
    torch.manual_seed(SEED)
    device = torch.device("cuda", 0)
    native_runtime = configure_native_runtime()
    if native_runtime != receipt["native_runtime"]:
        raise RuntimeError("E13 admission arithmetic differs from registration")
    if e13_environment_record(device) != receipt["environment"]:
        raise RuntimeError("E13 admission environment differs from registration")
