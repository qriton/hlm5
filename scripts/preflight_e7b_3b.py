"""Baseline-only preflight for the registered E7b native-admission study."""
...
def baseline_head_identity(
    ...
    if native_argmax != independent_argmax or max_abs > HEAD_IDENTITY_MAX_ABS:
        raise RuntimeError(
            "E7b baseline head identity failed: "
...
def main() -> None:
    ...
    if tracked != set(source_sha256):
        raise RuntimeError("every registered E7b source must be tracked by git")

    snapshot = snapshot_path()
    model_files = verify_snapshot(snapshot)
    if not torch.cuda.is_available() or torch.cuda.device_count() < 1:
        raise RuntimeError("registered E7b preflight requires one CUDA device")
    device = torch.device("cuda", 0)
    tokenizer = load_pinned_tokenizer(snapshot)
    e7_pool, fresh_pool = target_pools(tokenizer)
    ...
    if tests.returncode != 0:
        raise RuntimeError(
            "registered E7b tests failed before preflight:\n"
            + tests.stdout
            + tests.stderr
        )
    ...
    payload = {
        ...
        "e7_target_pool_count": len(e7_pool),
        "e7_target_pool_sha256": stable_json_sha256(e7_pool),
        ...
        "preflight_test_command": "python -m pytest tests/test_e7b_3b.py -q",
