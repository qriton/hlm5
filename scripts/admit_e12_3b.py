"""Run the single-use E12 nested-capacity admission."""
...
from hlm5.e12_contract import (
    ...
    CANDIDATE_CASE_SHA256, PROTOCOL_COMMIT, PROTOCOL_SHA256, QUERY_ROW_SHA256, RESULT_PATH,
...
def main() -> None:
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("registered E12 admission requires exactly one GPU")
    torch.manual_seed(SEED)
    device = torch.device("cuda", 0)
    native_runtime = configure_native_runtime()
    if native_runtime != receipt["native_runtime"]:
        raise RuntimeError("E12 admission arithmetic differs from registration")
    if e12_environment_record(device) != receipt["environment"]:
        raise RuntimeError("E12 admission environment differs from registration")
