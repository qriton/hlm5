"""Create the explicit pre-outcome execution receipt for E8."""
...
def main() -> None:
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("registered E8 registration requires exactly one GPU")
    ...
    if environment != preflight.get("environment"):
        raise RuntimeError("E8 registration environment differs from preflight")
    if native_runtime != preflight.get("native_runtime"):
        raise RuntimeError("E8 registration arithmetic differs from preflight")
    ...
    if tests.returncode != 0:
        raise RuntimeError(
            "registered E8 tests failed at registration:\n"
            + tests.stdout
            + tests.stderr
        )
    payload = {
    ...
    if payload["preflight_file_sha256_recomputed"] != preflight_sha:
        raise RuntimeError("E8 preflight changed during registration")
