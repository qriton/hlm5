"""New-process exact replay for registered HLM5 E12."""
...
from hlm5.e12_contract import (
    BUNDLE_PATH, RESULT_PATH, RESULT_SCHEMA, SEED, VERDICT_PATH,
    assert_outputs_absent, atomic_create_json, load_admission, load_attempt,
    load_execution_receipt, result_validity_failures, scientific_sha256,
    selected_population,
)
from hlm5.e12_runtime import (
    MODEL_FORWARD_BATCH, capacity_measurement, e12_environment_record, load_bundle,
)


def run_replay(
    *, started: float, admission: dict[str, Any], admission_sha: str,
    attempt_sha: str, native_runtime: dict[str, Any], device: torch.device,
) -> None:
    ...
    hidden = final_hidden_batch(model, tokenizer, prompts, device, batch_size=MODEL_FORWARD_BATCH)
    exact_hidden = hidden[: len(population["cases"])]
    whitening_hidden = hidden[len(population["cases"]):]
    locality_hidden = final_hidden_batch(
        model, tokenizer, population["query_prompt_order"], device,
        batch_size=MODEL_FORWARD_BATCH,
    )
    measurement, portable = capacity_measurement(
        model, population["cases"], exact_hidden, whitening_hidden,
        population["query_rows"], locality_hidden,
    )
    measurement["e12_population"] = {
    ...
    if frozen_bundle is not None:
        if portable is None:
            raise RuntimeError("E12 replay did not reconstruct the portable bundle")
...
def main() -> None:
    ...
    try:
        if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
            raise RuntimeError("registered E12 replay requires exactly one GPU")
        torch.manual_seed(SEED)
        device = torch.device("cuda", 0)
        native_runtime = configure_native_runtime()
        if native_runtime != receipt["native_runtime"]:
            raise RuntimeError("E12 replay arithmetic differs from registration")
        if e12_environment_record(device) != receipt["environment"]:
            raise RuntimeError("E12 replay environment differs from registration")
        run_replay(
            started=started, admission=admission, admission_sha=admission_sha,
            attempt_sha=attempt_sha, native_runtime=native_runtime, device=device,
        )
